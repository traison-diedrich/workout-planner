...
@router.patch("/", response_model=List[ExerciseReadWithInfo])
async def update_workout_exercises(*, session: Session = Depends(get_session), exercises: List[Exercise]):
    db_exercises = []
    for exercise in exercises:
        db_exercise = session.get(Exercise, exercise.id)
        if not db_exercise:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Exercise not found")
        exercise_data = exercise.dict(exclude_unset=True)
